...
var FINAL_POST = 3;
var NUM_DISCS = 9;
var EMPTY_POST_TEXT = 'Auf dem ausgewählten Turm befinden sich keine Scheiben.';
var INVALID_MOVE_HEADER = 'Du kannst die Scheibe nicht auf diesem Turm ablegen, ';
var INVALID_MOVE_FOOTER = ', weil die oberste Scheibe zu klein ist!';
var GAME_COMPLETE_TEXT = 'Herzlichen Grlückwunsch! Du hast gewonnen!\n\n Möchtest du noch einmal spielen?';
// "Class" variables
var posts = [];
posts[0] = [];
posts[1] = [];
		...
		if (selection === 0){ // Initial selection
			...
			if (selection !== post_num) { // Move disc
				if (isValidMove(disc_num,post_num)) {
					...
					$disc_to_move = changeLevel($disc_to_move, post, posts[post_num - 1]);
					$('#post_' + post_num).append($disc_to_move);
					finished = isPostFull(FINAL_POST);
					errorLog();
				} else {
					errorLog(INVALID_MOVE_HEADER + post_num + INVALID_MOVE_FOOTER);
				}
			} else {
				errorLog();
			}
...
function createDiscs() {
	...
	for (var index = NUM_DISCS; index >= 1; index--) {
		...
		$('<div />').addClass('disc disc_' + index + ' level_' + (NUM_DISCS - index + 1)).appendTo($post);
	}
}
